...
DT::datatable(tidy_Tena,caption="Tabla 1. Base de datos de número de peces ingresados, peso cosecha, y además número y porcentaje de mortalidad total, mortalidad Tenacibaculosis, mortalidad BKD y mortalidad SRS por Unidad")
...
hist_mort <- ggplot(tidy_Tena,aes(P.Mortalidad))+
  geom_histogram(fill="lightblue",color="black",bins=12)+
...
hist_BKD <- ggplot(tidy_Tena,aes(P.Mort_BKD))+
  geom_histogram(fill="lightblue",color="black",bins=10)+
  labs(title = "Mortalidad por BKD")

hist_SRS <- ggplot(tidy_Tena,aes(P.Mort_SRS))+
  geom_histogram(fill="lightblue",color="black",bins=10)+
...
bm <-ggplot(data=tidy_Tena, aes(x=Unidad,y=P.Mortalidad))+geom_boxplot()
bt <-ggplot(data=tidy_Tena, aes(x=Unidad,y=P.Mort_Tena))+geom_boxplot()
bb <-ggplot(data=tidy_Tena, aes(x=Unidad,y=P.Mort_BKD))+geom_boxplot()
bs <-ggplot(data=tidy_Tena, aes(x=Unidad,y=P.Mort_SRS))+geom_boxplot()
...
blot_mort <- ggplot(tidy_Tena,aes(y=P.Mortalidad,x=Alimento))+
  geom_boxplot(fill="lightgreen",outlier.colour = "red")+
...
blot_BKD <- ggplot(tidy_Tena,aes(y=P.Mort_BKD,x=Alimento))+
  geom_boxplot(fill="lightgreen",outlier.colour = "red")+
  labs(y="Mort. BKD",x="Tipo de Alimento")

blot_SRS <- ggplot(tidy_Tena,aes(y=P.Mort_SRS,x=Alimento))+
  geom_boxplot(fill="lightgreen",outlier.colour = "red")+
...
ip2 <- interaction.plot(tidy_Tena$Unidad,tidy_Tena$Alimento, tidy_Tena$P.Mortalidad,trace.label="Alimento",fun=mean,xlab="Unidad",ylab="% Mortalidad",type="b", col= c("black", "blue"),pch=c(19,17))
...
ip4 <- interaction.plot(tidy_Tena$Unidad,tidy_Tena$Alimento, tidy_Tena$P.Mort_BKD,trace.label="Alimento",fun=mean,xlab="Unidad",ylab="% Mortalidad BKD",type="b", col= c("black", "blue"),pch=c(19,17))

ip5 <- interaction.plot(tidy_Tena$Unidad,tidy_Tena$Alimento, tidy_Tena$P.Mort_SRS,trace.label="Alimento",fun=mean,xlab="Unidad",ylab="% Mortalidad SRS",type="b", col= c("black", "blue"),pch=c(19,17))
...
id2 <- plot.design(tidy_Tena$P.Mortalidad~tidy_Tena$Unidad*tidy_Tena$Alimento,xlab="Factores", ylab="% Mortalidad")
 
id3 <- plot.design(tidy_Tena$P.Mort_Tena~tidy_Tena$Unidad*tidy_Tena$Alimento,xlab="Factores", ylab="% Mortalidad Tenacibaculosis")

id4 <- plot.design(tidy_Tena$P.Mort_BKD~tidy_Tena$Unidad*tidy_Tena$Alimento,xlab="Factores", ylab="% Mortalidad BKD")

id5 <- plot.design(tidy_Tena$P.Mort_SRS~tidy_Tena$Unidad*tidy_Tena$Alimento,xlab="Factores", ylab="% Mortalidad SRS")
```


```{r, Tabla promedio  varianza ,echo=TRUE}
# Promedio y varianza de la variable % Mortalidad por Tenacibaculosis para los tipos de alimentos. 

Tabla = tidy_Tena %>% group_by(Alimento) %>%
summarize(N= n(), Mean = mean(P.Mort_Tena),
          Variance= var(P.Mort_Tena))

knitr::kable(Tabla,caption ="Estimadores puntuales de media y varianza de % Mortalidad por Tenacibaculosis para los tipos de alimentos")
```
