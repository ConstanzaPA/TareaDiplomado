...
**Gráficos de histograma y boxplot:**
Se describe la variación de cada variable de estudio usando histogramas y boxplot, como se muestra desde la figura 1 a la 8.
...
```{r Histograma y boxplot Peso promedio cosecha ,echo=TRUE}

hist_peso <- ggplot(tidy_Tena,aes(Peso_cosecha))+
  geom_histogram(fill="red",bins=12)+
  labs(title = "Figura 3. Histograma de peso promedio cosecha de los peces")
hist_peso
blot_peso <- ggplot(tidy_Tena,aes(y=Peso_cosecha,x=Alimento))+
  geom_boxplot(fill="red",outlier.colour = "red")+
  labs(title="Figura 4. Boxplot de peso promedio cosecha por tipo de alimento",y="Peso promedio cosecha",x="Tipo de Alimento")
blot_peso
```
...
```{r}
ggplot(data=tidy_Tena, aes(x=Unidad,y=Ingreso))+geom_boxplot()
ggplot(data=tidy_Tena, aes(x=Unidad,y=Ingreso))+geom_boxplot()
ggplot(data=tidy_Tena, aes(x=Unidad,y=Mortalidad))+geom_boxplot()
ggplot(data=tidy_Tena, aes(x=Unidad,y=Mort_Tena))+geom_boxplot()
ggplot(data=tidy_Tena, aes(x=Unidad,y=Mort_BKD))+geom_boxplot()
ggplot(data=tidy_Tena, aes(x=Unidad,y=Mort_SRS))+geom_boxplot()
```
...
Desde la figura 9 a la 14 se muestran gráficos de interacción entre cada variable cuantitativa y los factores.
...
interaction.plot(tidy_Tena$Centro,tidy_Tena$Alimento, tidy_Tena$Peso_cosecha,trace.label="Alimento",fun=mean,main="Figura 9. Interacción entre peso promedio cosecha y factores", xlab="Centro",ylab="Peso promedio cosecha (Kg)",type="b", col= c("black", "red"),pch=c(19,17))
 
plot.design(tidy_Tena$Peso_cosecha~tidy_Tena$Centro*tidy_Tena$Alimento,main="Figura 10. Gráfico de interacción",xlab="Factores", ylab="Peso promedio cosecha")
```
